...
By definition there are well known topological relationships between the various types of zones. In particular (see also <<tg-au-classMUOverview, Figure {counter:pic-id}>> and <<tg-au-recommendationMaritimeUnits, Recommendation {counter:rec-id}>>):
...
[PERMISSION, id="Recommendation {rec-id}"]
======
The following geometric and topological constraints are recommendations for this data specification:
